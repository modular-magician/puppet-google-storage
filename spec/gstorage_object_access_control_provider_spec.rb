...
describe Puppet::Type.type(:gstorage_object_access_control).provider(:google) do
  before(:all) do
  ...
  context '#flush' do
    subject do
      Puppet::Type.type(:gstorage_object_access_control).new(
        ensure: :present,
        name: 'my-name'
      ).provider
    end
  ...
  def load_network_result(file)
    results = File.join(File.dirname(__FILE__), 'data', 'network',
                        'gstorage_object_access_control', file)
  ...
  def create_type(id)
    Puppet::Type.type(:gstorage_object_access_control).new(
      ...
      bucket: GoogleTests::Constants::OAC_BUCKET_DATA[(id - 1) \
        % GoogleTests::Constants::OAC_BUCKET_DATA.size],
      object: GoogleTests::Constants::OAC_OBJECT_DATA[(id - 1) \
        % GoogleTests::Constants::OAC_OBJECT_DATA.size],
      entity: GoogleTests::Constants::OAC_ENTITY_DATA[(id - 1) \
        % GoogleTests::Constants::OAC_ENTITY_DATA.size]
    )
  end

  def expand_variables(template, data, extra_data = {})
    Puppet::Type.type(:gstorage_object_access_control).provider(:google)
